...
set minDiameter=170.0

set maxDiameter=190.0

set j = `echo "$diameterStep $minDiameter $maxDiameter" | awk '{printf ("%.0f\n", ($3 - $2)/$1)}'`

echo "This will have $j steps going from $minDiameter to $maxDiameter . See progress below."
...
while ( $k <= $j )
	...
	echo "Now on iteration $k.Out of $j. Diameter:$minDiameter"
end



#echo "The max value of Figure of merit is $max happening at a diameter parameter of $dia"
#set k=0

#set minDiameter = `expr $max -5`
#set maxDiameter = `expr $max +5`
#set DiameterStep=0.1

#while ( $k <= $j )

 #       /opt/EM_suite/Gautomatch_v0.53/bin/Gautomatch-v0.53_sm_20_cu6.5_x86_64 --apixM 1.54 --diameter $minDiameter $file >/dev/null
    ...
    #    set avrg_ccc=`awk -v N=5 '{ sum += $N } END { if (NR > 0) print sum / NR }' $starFile | xargs echo` #Calculates the Average of the Column N
     ...
     #   echo "$minDiameter  $avrg_ccc  $var" >> excel.csv
      ...
      #  @ k++
       ...
       # echo "Progress:$percent%.  Now on iteration $k. Diameter:$minDiameter."
